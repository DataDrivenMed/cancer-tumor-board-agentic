...
def inject_structure_css() -> None:
    st.markdown(
        """
<style>
.agent-shell{max-width:1240px;margin:0 auto}.agent-hero{padding:7px 0 20px;border-bottom:1px solid var(--line);margin-bottom:22px}.agent-hero h1{font-size:47px;line-height:1.02;margin:6px 0 8px}.agent-hero p{max-width:840px;color:var(--body);font-size:15px;line-height:1.62;margin:0}
.agent-turn{display:flex;gap:12px;margin:0 0 18px}.agent-avatar{width:31px;height:31px;flex:none;border-radius:8px;display:grid;place-items:center;font:700 10px/1 var(--mono);background:var(--panelhi);border:1px solid var(--line);color:var(--accent2)}.agent-avatar.user{background:var(--accent);color:#231a13;border-color:var(--accent)}.agent-bubble{flex:1;min-width:0}.agent-who{font:600 10px/1 var(--mono);text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin:4px 0 7px}.agent-text{font-size:14px;line-height:1.64;color:var(--body)}.user-msg{background:var(--panelhi);border:1px solid var(--line);border-radius:12px;padding:12px 14px;color:var(--ink)}
...
.inspector{position:sticky;top:1rem}.inspect-head{font:600 10px/1 var(--mono);text-transform:uppercase;letter-spacing:.1em;color:var(--muted);margin:0 0 9px}.inspect-card{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px;margin-bottom:9px}.inspect-card.primary{border-color:var(--linehi);background:var(--panelhi)}.inspect-row{display:flex;justify-content:space-between;gap:8px;padding:5px 0;border-bottom:1px solid var(--line2);font-size:11.5px}.inspect-row:last-child{border-bottom:0}.inspect-k{color:var(--muted)}.inspect-v{color:var(--ink);text-align:right;font-weight:600;max-width:58%}.inspect-v.okv{color:var(--mint)}.inspect-v.warnv{color:var(--warn)}.inspect-note{font-size:10.5px;line-height:1.48;color:var(--muted);margin-top:7px}.inspect-legend{display:flex;gap:4px;flex-wrap:wrap;margin-top:8px}
...
.logic-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:0 0 18px}.logic-cell{border:1px solid var(--line2);background:var(--panel2);border-radius:10px;padding:10px 11px}.logic-k{font:600 9px/1 var(--mono);text-transform:uppercase;letter-spacing:.07em;color:var(--muted)}.logic-v{font-size:12.5px;color:var(--ink);font-weight:600;margin-top:5px}
@media(max-width:1050px){.fact-grid{grid-template-columns:1fr}.agent-hero h1{font-size:40px}.logic-strip{grid-template-columns:repeat(2,1fr)}}@media(max-width:700px){.logic-strip{grid-template-columns:1fr}}
</style>
""",
...
def sidebar(stage: str, reset_callback) -> None:
    ...
    with st.sidebar:
        st.markdown('<div class="fx-side-brand"><div class="fx-side-mark">TB</div><div><div class="fx-side-name">Tumor Board</div><div class="fx-side-sub">Governed agentic workup</div></div></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="small-note" style="margin:10px 3px 18px">The agent advances automatically through evidence gathering and synthesis. Human action is required at explicit representation and evidence guardrails.</div>', unsafe_allow_html=True)
...
def case_facts(case) -> None:
    ...
    molecular = ", ".join(
        " ".join(x for x in [m.gene, m.alteration_type or m.hgvs_p or m.hgvs_c] if x)
        for m in case.molecular_findings[:3]
    ) or "Not documented"
    facts = [
    ...
    for key, value, item in facts:
        ...
        html.append(
            f'<div class="fact-card"><div class="fact-key">{escape(key)}</div>'
            f'<div class="fact-value">{escape(txt(value))}</div>'
            f'<div class="fact-meta">{escape(meta)}</div></div>'
        )
    st.markdown('<div class="fact-grid">' + "".join(html) + '</div>', unsafe_allow_html=True)


def logic_strip(result: dict[str, Any]) -> None:
    """Compact center-column explanation of why the governed result is supportable."""
    routing = result.get("routing")
    integrity = result.get("case_integrity_report")
    missing = result.get("missing_information_report")
    final = result.get("final_decision")
    values = [
        ("Integrity", human(val(integrity, "disposition", "not run"))),
        ("Missing info", human(val(missing, "disposition", "not run"))),
        ("Agents routed", str(len(val(routing, "selected_agents", []) or []))),
        ("Decision support", human(val(final, "decision_support_strength", "insufficient"))),
    ]
    html = "".join(
        f'<div class="logic-cell"><div class="logic-k">{escape(k)}</div><div class="logic-v">{escape(v)}</div></div>'
        for k, v in values
    )
...
def inspector() -> None:
    ...
    brief = result.get("tumor_board_brief") if isinstance(result, dict) else None
    audit = result.get("audit_events", []) if isinstance(result, dict) else []
    runtime = st.session_state.ag_runtime_status or {}
    evidence = st.session_state.get("ag_evidence_summary", {}) or {}

    st.markdown('<div class="inspector"><div class="inspect-head">Live workup inspector</div>', unsafe_allow_html=True)

    final_state = human(val(final, "decision_state", "pending")) if result else "Pending"
    st.markdown(
        '<div class="inspect-card primary">'
        f'<div class="inspect-row"><span class="inspect-k">Stage</span><span class="inspect-v">{escape(STAGE_LABELS[stage])}</span></div>'
        f'<div class="inspect-row"><span class="inspect-k">Decision state</span><span class="inspect-v">{escape(final_state)}</span></div>'
        f'<div class="inspect-row"><span class="inspect-k">Pathway</span><span class="inspect-v">{escape(validation.label)}</span></div>'
        f'<div class="inspect-row"><span class="inspect-k">Common core</span><span class="inspect-v okv">{"Qualified" if validation.common_core_qualified else "Not qualified"}</span></div>'
        f'<div class="inspect-row"><span class="inspect-k">Clinical validation</span><span class="inspect-v warnv">{"Established" if validation.clinically_validated else "Not established"}</span></div>'
        '<div class="inspect-note">Software qualification and clinical validation are separate claims. Clinical release is not established.</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if case is not None:
        ...
        st.markdown(
            '<div class="inspect-card"><div class="inspect-head">Case representation</div>'
            ...
            f'<div class="inspect-row"><span class="inspect-k">Case type</span><span class="inspect-v">{escape(human(case.case_type))}</span></div></div>',
            unsafe_allow_html=True,
        )

    if evidence and (evidence.get("molecular_candidates") or evidence.get("safety_candidates") or st.session_state.ag_evidence_confirmed):
        st.markdown(
            '<div class="inspect-card"><div class="inspect-head">Evidence commissioning</div>'
            f'<div class="inspect-row"><span class="inspect-k">Molecular candidates</span><span class="inspect-v">{int(evidence.get("molecular_candidates", 0))}</span></div>'
            f'<div class="inspect-row"><span class="inspect-k">Molecular attested</span><span class="inspect-v">{int(evidence.get("molecular_attested", 0))}</span></div>'
            f'<div class="inspect-row"><span class="inspect-k">Safety candidates</span><span class="inspect-v">{int(evidence.get("safety_candidates", 0))}</span></div>'
            f'<div class="inspect-row"><span class="inspect-k">Safety attested</span><span class="inspect-v">{int(evidence.get("safety_attested", 0))}</span></div>'
            f'<div class="inspect-row"><span class="inspect-k">Human gate</span><span class="inspect-v">{"Complete" if st.session_state.ag_evidence_confirmed else "Pending"}</span></div>'
            '<div class="inspect-note">Retrieved evidence is not admitted to patient-level molecular or safety reasoning until explicitly attested.</div></div>',
            unsafe_allow_html=True,
        )
    ...
    if runtime:
        ...
        for channel in ("guideline", "molecular", "translational", "safety", "pubmed", "clinical_trials"):
            ...
            state = "Ready" if ready else "Fail closed"
            cls = "okv" if ready else "warnv"
            rows.append(
                f'<div class="inspect-row"><span class="inspect-k">{escape(channel.replace("_", " ").title())}</span>'
                f'<span class="inspect-v {cls}">{state}</span></div>'
            )
        st.markdown(
            '<div class="inspect-card"><div class="inspect-head">Evidence runtime</div>'
            + "".join(rows)
            + '<div class="inspect-note">A failed evidence channel cannot silently fall back to model memory.</div></div>',
            unsafe_allow_html=True,
        )

    if result:
        selected = val(routing, "selected_agents", []) or []
        blocking = val(missing, "blocking_count", 0) or 0
        safe_display = bool(val(brief, "safe_to_display", False))
        st.markdown(
            '<div class="inspect-card"><div class="inspect-head">Governance gates</div>'
            ...
            f'<div class="inspect-row"><span class="inspect-k">Blocking gaps</span><span class="inspect-v">{blocking}</span></div>'
            ...
            f'<div class="inspect-row"><span class="inspect-k">Brief safe to display</span><span class="inspect-v">{"Yes" if safe_display else "No"}</span></div>'
            f'<div class="inspect-row"><span class="inspect-k">Audit events</span><span class="inspect-v">{len(audit)}</span></div></div>',
            unsafe_allow_html=True,
        )

        if selected:
            st.markdown(
                '<div class="inspect-card"><div class="inspect-head">Specialists routed</div>'
                f'<div class="inspect-note">{escape(" · ".join(human(x) for x in selected))}</div></div>',
                unsafe_allow_html=True,
            )
        ...
        if audit:
            latest = audit[-1]
            st.markdown(
                '<div class="inspect-card"><div class="inspect-head">Latest audit event</div>'
                f'<div class="inspect-note">{escape(txt(val(latest, "event_type", val(latest, "event", "event"))))}<br>'
                f'{escape(txt(val(latest, "detail", val(latest, "message", "")), ""))}</div></div>',
                unsafe_allow_html=True,
            )

    st.markdown(
        '<div class="inspect-card"><div class="inspect-head">Epistemic model</div>'
        '<div class="inspect-legend">'
        + claim_chip("source")
        + claim_chip("retrieved")
        + claim_chip("derived")
        + claim_chip("human")
        + '</div><div class="inspect-note">These categories are intentionally not interchangeable. Retrieval does not become a patient fact, and interpretation does not become human judgment.</div></div>',
        unsafe_allow_html=True,
    )
