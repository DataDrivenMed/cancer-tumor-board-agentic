...
def parse_upload(filename: str | Any, raw_bytes: bytes | None = None, document_id: str = "DOC-001") -> ParsedDocument:
    ...
    segments: list[SourceSegment] = []
    if suffix == ".pdf":
        reader = PdfReader(BytesIO(raw_bytes)); counter = 1
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            for paragraph_number, paragraph in enumerate(paragraphs, start=1):
                ...
                segments.append(SourceSegment(segment_id=f"S{counter:04d}", text=text, page=page_number, paragraph=paragraph_number)); counter += 1
        return ParsedDocument(document_id, filename, "pdf", segments)
    if suffix == ".docx":
        doc = Document(BytesIO(raw_bytes)); counter = 1
        for paragraph_number, paragraph in enumerate(doc.paragraphs, start=1):
            text = _clean(paragraph.text)
            if not text: continue
            segments.append(SourceSegment(segment_id=f"S{counter:04d}", text=text, paragraph=paragraph_number)); counter += 1
        return ParsedDocument(document_id, filename, "docx", segments)
    if suffix in {".txt", ".md"}:
        text = raw_bytes.decode("utf-8", errors="replace"); counter = 1
        for paragraph_number, paragraph in enumerate(text.splitlines(), start=1):
            cleaned = _clean(paragraph)
            if not cleaned: continue
            segments.append(SourceSegment(segment_id=f"S{counter:04d}", text=cleaned, paragraph=paragraph_number)); counter += 1
...
def parse_text(text: str, document_id: str = "DOC-001", filename: str = "pasted_case.txt") -> ParsedDocument:
    return parse_upload(filename, text.encode("utf-8"), document_id=document_id)
