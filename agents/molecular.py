...
def _record_matches(case: CancerTumorBoardCase, finding: MolecularFinding, record: MolecularEvidenceRecord) -> bool:
    ...
    if record.alteration_terms:
        ...
        if not any(any(_norm(term) in fterm or fterm in _norm(term) for fterm in finding_terms) for term in record.alteration_terms if _norm(term)):
            return False
    return True
    ...
    def run(self, case: CancerTumorBoardCase) -> MolecularReport:
        if not is_registered_oncology_program(case.disease_program):
            return MolecularReport(
                case_id=case.case_id, status="abstain_domain",
        ...
        if not case.molecular_findings:
            return MolecularReport(
                case_id=case.case_id, status="no_evidence_found",
        ...
        usable_records = [r for r in self.store.records if r.source_verified and r.human_verified and not (self.production_mode and r.synthetic)]
        if not usable_records:
            return MolecularReport(
                case_id=case.case_id, status="source_unavailable",
        ...
        for finding in case.molecular_findings:
            ...
            established_or_emerging = actionability in {ClinicalActionability.ESTABLISHED, ClinicalActionability.EMERGING}
            any_actionable = any_actionable or established_or_emerging
            interpretations.append(MolecularFindingInterpretation(
                gene=finding.gene,
                ...
                limitations=[] if matched else ["No verified disease- and alteration-matched evidence record was found; no actionability inference was made."],
        ...
        return MolecularReport(
            case_id=case.case_id, status=status, interpretations=interpretations,
            limitations=[
            ...
            summary=summary, can_support_clinical_actionability_claim=any_actionable,
        )
