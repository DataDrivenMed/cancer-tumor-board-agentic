...
def _make_item(
    *, field: str, category: str, priority: MissingInformationPriority, reason: str,
    availability: str, recommendation_blocking: bool, action: MissingInformationAction,
    source: str, field_path: str | None = None, source_segment_ids: list[str] | None = None,
) -> MissingInformationItem:
    return MissingInformationItem(
        item_id=_item_id(field, category, reason), field=field, category=category,
        priority=priority, reason=reason, availability=availability,
        recommendation_blocking=recommendation_blocking, action=action, source=source,
        field_path=field_path, source_segment_ids=sorted(set(source_segment_ids or [])),
...
def _existing_missing_items(case: CancerTumorBoardCase) -> list[MissingInformationItem]:
    ...
    for idx, item in enumerate(case.missing_items):
        ...
        items.append(_make_item(
            field=item.field, category=category, priority=priority, reason=item.reason,
            availability=item.availability, recommendation_blocking=item.recommendation_blocking,
            action=_action_for(availability=item.availability, category=category),
            source="canonical_missing_item", field_path=f"missing_items[{idx}]",
        ))
    return items
...
def _structural_items(case: CancerTumorBoardCase) -> list[MissingInformationItem]:
    ...
    if _status_unresolved(case.disease_state.status) or case.disease_state.value in {None, ""}:
        items.append(_make_item(
            ...
            source="structural_rule", field_path="disease_state", source_segment_ids=_segments_from_fact(case.disease_state),
        ))
    stage = case.stage
    if stage is not None and (_status_unresolved(stage.status) or stage.value in {None, ""}):
        ...
        items.append(_make_item(
            field="cancer stage", category="stage",
            priority=MissingInformationPriority.CRITICAL if stage_conflicting else MissingInformationPriority.HIGH,
            reason=("Distinct explicit stage statements are represented and require source or temporal reconciliation before management synthesis." if stage_conflicting else "Stage is explicitly represented as unresolved and requires review before it is used in tumor-board reasoning."),
            availability=stage.status.value, recommendation_blocking=stage_conflicting,
            action=MissingInformationAction.RESOLVE_CONFLICT if stage_conflicting else (MissingInformationAction.REVIEW if stage.status == DataStatus.PENDING else MissingInformationAction.OBTAIN),
            source="structural_rule", field_path="stage", source_segment_ids=_segments_from_fact(stage),
        ))
    ...
    if ps is None or _status_unresolved(ps.status) or ps.value in {None, ""}:
        ...
        items.append(_make_item(
            field="performance status", category="performance_status", priority=MissingInformationPriority.HIGH,
            reason="Performance status is unresolved and can affect treatment intensity, tolerability assessment, and trial screening.",
            availability=availability, recommendation_blocking=False, action=MissingInformationAction.OBTAIN,
            source="structural_rule", field_path="performance_status",
    ...
    if relapsed_like and not case.treatments:
        items.append(_make_item(
            field="prior treatment history", category="treatment_history", priority=MissingInformationPriority.CRITICAL,
            reason="Relapsed, refractory, or progressive disease is represented without prior treatment history.",
            availability="not_documented", recommendation_blocking=True, action=MissingInformationAction.OBTAIN,
            source="structural_rule", field_path="treatments",
        ))
    ...
    if trial_or_treatment and case.disease_program == "hematologic_malignancy" and not case.molecular_findings:
        items.append(_make_item(
            field="molecular/cytogenetic characterization", category="molecular", priority=MissingInformationPriority.MODERATE,
            ...
            availability="not_documented", recommendation_blocking=False, action=MissingInformationAction.REVIEW,
            source="structural_rule", field_path="molecular_findings",
        ))
    return items
...
def _conflict_items(case: CancerTumorBoardCase) -> list[MissingInformationItem]:
    ...
    for idx, conflict in enumerate(case.conflicts):
        if conflict.resolution_status != "unresolved":
            continue
        priority = MissingInformationPriority.CRITICAL if conflict.severity == "critical" else (MissingInformationPriority.HIGH if conflict.severity == "high" else MissingInformationPriority.MODERATE)
        blocking = conflict.severity in {"high", "critical"}
        items.append(_make_item(
            field=f"resolve conflict: {conflict.field}", category="conflict_resolution", priority=priority,
            reason=f"Conflicting source values remain unresolved: {conflict.value_a!r} versus {conflict.value_b!r}.",
            availability="conflicting", recommendation_blocking=blocking, action=MissingInformationAction.RESOLVE_CONFLICT,
            source="conflict_rule", field_path=f"conflicts[{idx}]", source_segment_ids=conflict.source_segment_ids,
        ))
    return items
...
def _deduplicate(items: list[MissingInformationItem]) -> list[MissingInformationItem]:
    ...
    for item in items:
        ...
        if current is None or rank[item.priority.value] > rank[current.priority.value] or (rank[item.priority.value] == rank[current.priority.value] and item.recommendation_blocking and not current.recommendation_blocking):
            best[key] = item
    return sorted(best.values(), key=lambda x: (-x.priority_score, not x.recommendation_blocking, x.category, _norm(x.field), x.item_id))


def run_missing_information(case: CancerTumorBoardCase) -> MissingInformationReport:
    items = _deduplicate(_existing_missing_items(case) + _structural_items(case) + _conflict_items(case))
    ...
    return MissingInformationReport(
        case_id=case.case_id, disposition=disposition, items=items, blocking_count=blocking_count,
        critical_count=critical_count, high_count=high_count, moderate_count=moderate_count, low_count=low_count,
        safe_to_route_to_specialists=blocking_count == 0, requires_human_review=bool(items), summary=summary,
    )
